...
### ConcertEntry — 공연 응모 (추첨 신청)

`id` · `scheduleId` FK · `userId` FK · `createdAt`

`(scheduleId, userId)` **UNIQUE** — 이 제약이 곧 **1인 1공연 1매** 규칙입니다.
매수 컬럼이 없습니다. 디자인 확정본이 「응모하기」 버튼 하나라서 고를 것이 없습니다.

⚠️ **결제가 아닙니다.** 당첨 이후 실제 예매는 공식 예매처에서 이뤄지고 플랫폼은 금전
거래를 중개하지 않습니다 (기획서 정책). 화면에 그 고지가 반드시 남아 있어야 합니다.

`Gathering` 과 달리 **정원 카운터가 없어** 원자적 UPDATE 도 비관적 락도 필요 없습니다.
중복만 막으면 됩니다. 취소는 상태를 남기지 않고 행을 지웁니다.

---

### ArtistStage — 아티스트별 대표 무대 영상

`id` · `artistName` **UNIQUE** · `title` · `embedUrl` · `thumbnailUrl?`

"이찬원 무대 보여줘" 라는 음성 요청의 답입니다. 랜딩에서 고를 수 있는 13명과 1:1 이며
`artistName` 은 `FE/src/data/programs.ts` 의 이름과 **한 글자도 다르면 안 됩니다.**

⚠️ **URL 을 LLM 에게 생성시키지 않는 이유**가 이 표의 존재 이유입니다. 모델이 지어낸
YouTube ID 는 없는 영상이거나 임베드 차단 영상이고, 차단 영상은 오류 없이 **검은 화면 +
스피너**만 남기고 조용히 실패합니다. AI 는 "무대 보여줘" 라는 의도와 아티스트 이름만
판정하고, 주소는 여기서 옵니다.

`embedUrl` 은 `watch?v=` 가 아니라 **`embed/` 형식**이며, 교체할 때는 반드시 **iframe 안에서**
재생을 확인하세요 (주소창에 직접 여는 검증은 무의미 — 정상 영상도 오류 153).
