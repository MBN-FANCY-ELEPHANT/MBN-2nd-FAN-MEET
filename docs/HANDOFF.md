...
**최종 갱신:** 2026-08-08 · **시드 아티스트를 3명(성리·이찬원·박서진)으로 재구성.**
`data.sql` 전면 개편(약 150행) + 이름 목록 3곳 정합. DB·API 는 아티스트별로 완전히 분리됐고
검증도 통과했지만, **FE `STAR_ID = 1` shim 이 남아 있어 화면에는 아직 성리만 보입니다** —
남은 작업은 `docs/DB구성.md` §5.
