...
4. **응원 아티스트 이름** — "이찬원 공연 응모해줘" 는 특정 공연을 지목한 것이 아니라
   "내 아티스트의 공연" 이라는 뜻입니다. 단서로 두면 어느 일정 제목과도 안 맞아 실패합니다.
...
| "가장 가까운 공연에 대한 표를 응모해줘" | 그 아티스트의 가장 가까운 일정 ✅ (기본값) |
...
## 2-3. 시드 아티스트 3명 재구성 (2026-08-08)

`data.sql` 을 **성리(1) · 이찬원(2) · 박서진(3)** 3명 기준으로 전면 개편했습니다.
설계 근거·필드별 필수 여부·운영 DB 반영 절차는 **`docs/DB구성.md`** 에 따로 정리했습니다.

**구성** — 공통(channel 3, app_user 9) + 아티스트당 schedule 5 · content 11
(ARTICLE 3 / VIDEO 4 / POST·STAR 2 / POST·MANAGER 2) · comment 11 · place 5 · tip 6 ·
gathering 4 · artist_stage 1. 총 약 150행.

**ID 대역을 아티스트별로 나눴습니다** — 성리 1~10 / 이찬원 21~30 / 박서진 41~50
(schedule·place·tip 은 건수가 적어 1~10 / 11~20 / 21~30). 대역을 침범하지 마세요.

**함께 고친 이름 목록 3곳** — 이 목록들은 `star.name` 을 그대로 따라가야 합니다.

| 파일 | 무엇 | 안 고치면 |
|---|---|---|
| `EvidenceFinder.DOMAIN_WORDS` | `"임영웅"` → 3명 이름 | "박서진 요즘 뭐해?" 가 스코프 밖으로 거절 |
| `ChatService.SUGGESTED_KO/EN` | 예시 질문에서 아티스트 이름 제거 | 추천 질문에 남의 이름이 뜸 |
| `FE/src/data/programs.ts` | 13명 → 3명 | 데이터 없는 아티스트를 고르면 빈 화면 |

**검증 (실측)**

- 기동 6.7초, 시드 오류 0. AI 워밍업 콘텐츠 33건 × 2언어 = 66건 / 94초(백그라운드)
- 3명 각각 다가오는 일정 5건, 전부 미래
- `content.comment_count` ↔ 실제 댓글 **33건 전수 일치**
- 음성 액션이 아티스트별로 자기 데이터만 잡음
  (무대 3/3, 공연 응모 → 1·11·21, 버스 대절 → 1·11·21. **교차 오염 0**)

⚠️ **남은 일 — FE 단일 스타 shim.** `STAR_ID = 1` 이 20개 파일에 고정돼 있어
**앱에서 누구를 골라도 성리 데이터만 보입니다.** `personalizeArtistNames()` 와
`ChatService.rewriteArtist()` 는 데이터가 1명뿐이라 존재하던 shim 이라, 이제는
**정확한 값을 틀린 값으로 바꿉니다.** 제거 순서는 `docs/DB구성.md` §5.
