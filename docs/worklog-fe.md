...
## 0-4. 팬공간 투표 (2026-08-08)

팬공간 상단 배너가 열던 `/fanspace/vote` 가 빈 화면(`EmptyMascotState`)이었습니다.
**Figma 27:6676(미선택) · 27:6705(선택)** 을 옮겨 `VoteDetailPage` 로 구현했습니다.

**세 아티스트가 같은 투표를 봅니다** — 한일가왕전 16강전은 아티스트별로 갈리는 이벤트가
아니라 하나의 경연이라, 선택 결과도 아티스트별이 아닌 **하나의 키**에 저장합니다.

**BE 무관** — 투표 도메인이 없어 데이터·상태가 전부 FE 에 있습니다 (`data/vote.ts`).
배포 백엔드·DB 를 못 건드리는 상황에서도 **FE 재배포만으로** 동작합니다.

| 상태 | 화면 |
|---|---|
| 미선택 | 라디오 전부 회색, 「투표하기」 `--color-text-secondary` 비활성 |
| 선택 | 고른 항목만 주황(라디오·라벨), 「투표하기」 `--color-primary` 활성 |
| 투표 완료 | 배지 "투표 완료", 득표율 막대 공개, 선택 잠금 (Figma 에 없는 상태 — 신설) |

⚠️ **득표율·참여 수는 시연용 고정값입니다** (`CURRENT_VOTE.candidates[].share`).
실제 집계가 아니고 서버로 아무것도 보내지 않습니다. 선택은 `localStorage` 라 **이 기기에만**
남습니다 — 다른 기기로 시연하면 미투표 상태로 시작합니다.

**밟은 함정 — 하단 고정 CTA 는 `--app-width` 를 써야 합니다.**
`max-width: 430px; margin: 0 auto` 로 두면 `margin:auto` 가 **뷰포트** 기준이라,
`#root` 가 375px 로 좁아진 화면에서 버튼이 **좌우로 28px 씩 삐져나옵니다**(실측).
`left: 50% + translateX(-50%) + max-width: var(--app-width)` 로 해야 캔버스에 붙습니다 —
굿즈 상세 `.actionBar` 가 쓰는 방식과 같습니다.

**아이콘** — 라디오 on/off 가 프로젝트에 없어 Figma 에셋을 받아 커밋했습니다
(`assets/icons/radio-{on,off}.svg`, `Icon` 에 등록). 글리프는 같고 fill 만 다릅니다
(`#737373` / `#F58220`). ⚠️ 직접 `<svg>` 를 그리지 마세요 — 원본 벡터가 없으면 반드시 어긋납니다.

**라우트 순서** — `/fanspace/vote` 는 `/fanspace/:category` **앞에** 있어야 합니다.
뒤에 두면 `category="vote"` 로 잡혀 다시 빈 화면이 됩니다.


## 0-5. 공연 사진 — 비율 유지 + 아티스트 사진 순환 (2026-08-08)

`schedule` 에 이미지 컬럼이 없어 공연 화면은 아티스트 폴더의 사진을 씁니다.
한 장(`concert`)만 돌려 쓰다 보니 목록 전체가 같은 그림이었습니다.

**`artistPhoto(name, seed)`** (`data/programs.ts`) 를 추가해 **일정 id 로 번갈아** 고릅니다.
`ARTISTS[].photos` 에 각 아티스트가 실제로 넣어둔 파일명이 들어 있습니다
(`concert` → `post-1` → `profile` 순, 확장자가 섞여 있어 파일명 그대로 적습니다).

**id 를 seed 로 쓰는 이유** — 목록 카드와 상세 화면이 **같은 사진**을 쓰게 됩니다.
인덱스나 난수를 쓰면 눌러 들어갔을 때 그림이 바뀌어 다른 공연처럼 보입니다.
새로고침해도 안 바뀌는 것도 같은 이유입니다.

| 화면 | 잘림 처리 | 이유 |
|---|---|---|
| 공연 카드 (`.concertPoster`) | `height: auto` | 카드가 화면 폭을 다 써서 비율에 맡겨도 무너지지 않음 |
| 공연 상세 (`.poster`) | `height: auto` | 목록과 **같은 사진**이라 처리도 같아야 함 |
| 응모 캐러셀 (`.miniThumb`) | `height: 138px` + `object-fit: contain` | 가로로 늘어서는 카드라 `auto` 면 카드 높이가 들쭉날쭉해짐 |

⚠️ **비율을 살리면 카드가 길어집니다.** 세로 사진(800×1200)이면 카드 사진만 512px 이라
한 화면에 한 장 남짓 보입니다. 목록 밀도가 더 중요해지면 `.concertPoster` 도
캐러셀처럼 `고정 높이 + contain` 으로 바꾸세요 (잘림 없이 압축됩니다).

**실측 (박서진 5건)** — `concert.jpg` → `post-1.jpg` → `profile.png` → `concert.jpg` → `post-1.jpg`,
전부 비율 유지. 상세(22번)도 목록과 같은 `post-1.jpg`. 캐러셀 카드 높이 208px 로 균일.
