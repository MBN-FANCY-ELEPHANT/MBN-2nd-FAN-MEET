...
**최종 갱신:** 2026-08-08 · **게스트 팬 식별·음성 액션 5종 및 아티스트 SNS형 POST 통합.**
스타 선택→팬 참여 통합 테스트 통과, 모집 중복 신청 버그 수정, POST를 기존 1단계 댓글 API와 연결.
...
⚠️ **다만 지금 이 파일에서 화면 노드는 조회되지 않습니다.** MCP 가 `components` 페이지
하나만 노출해서 `7:1432`(음성 오버레이)·`22:4264`(팬공간) 등이 전부 "not found" 입니다.
`component-map.md` 의 노드 ID 도 그래서 **검증 불가** 상태입니다 — 재실측이 필요하면
사용자에게 **새 노드 URL** 을 받으세요.
...
| **Figma 화면 노드를 더 이상 못 읽습니다** | MCP 에 `components` 페이지만 남아 있어 `7:1432`(음성) 등 화면 노드가 조회되지 않습니다. 재실측이 필요하면 **새 노드 URL 을 받아야 합니다** |
